...
creds = ServiceAccountCredentials.from_json_keyfile_name(
    r"D:\matching_harsh\Job_matching_Screened\candidate_jobs_formate\service_account.json", scope
)
...
for _, row in df.iterrows():
    ...
    cursor.execute("""
        ...
        SET lineupId = %s,
            ...
            lineup_finploy_hr = %s
        WHERE phone_no = %s
          AND button_response = 'interested'
    """, (
        row["candidate_id"],
...
import subprocess
subprocess.run(
    ["python", r"D:\matching_harsh\Job_matching_Screened\candidate_jobs_formate\hr_name.py"],
    check=True
)
