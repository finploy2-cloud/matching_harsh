...
input_file = r"D:\matching_harsh\Screening_Followup\output\output1.xlsx"
...
# =====================================================
# 2️⃣ clean_salary column
# =====================================================
salary_col = colmap.get("salary", None)
clean_col = colmap.get("clean_salary", None)

if salary_col:
    salary_series = df[salary_col].astype(str)
elif clean_col:
    salary_series = df[clean_col].astype(str)
else:
    salary_series = pd.Series([""] * len(df))
    print("⚠️ Neither 'Salary' nor 'clean_salary' column found. Creating blank clean_salary.")
...
try:
    ...
    subprocess.run(["python", r"D:\matching_harsh\Screening_Followup\main4.py"], check=True)
